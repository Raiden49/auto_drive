    ...
    if (!common_info_ptr_->global_path_.empty() && 
      ...
      if (local_path_.size() > 1) {
        ...
        if (is_planner_) {
          ...
          else if (fabs(common_info_ptr_->cur_pose_.x - 
                        pre_final_path_.frenet_points[0].x) > 2.0 ||
                   fabs(common_info_ptr_->cur_pose_.y - 
                        pre_final_path_.frenet_points[0].y) > 0.5) {
            double dt = 0.1;
          ...
          auto initial_frenet_point = CalFrenet(global_initial_point, ref_path_);
